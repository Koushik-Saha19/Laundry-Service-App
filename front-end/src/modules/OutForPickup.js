...
class OutForPickup extends Component {
    ...
    refreshProduct(){
        Dataservice.OutForPickup().then(response =>{
            this.setState({
    ...
    pickedUp(custUsername,orderId,typeOfService,quantity,orderStatus){
        this.props.history.push(`/PickUpUpdate/${custUsername}/${orderId}/${typeOfService}/${quantity}/${orderStatus}`)
    ...
    render() {
        return (
            ...
            <h1><center>Out For Pickup</center></h1>
                                        ...
                                        <tbody>
                                            {customer.order.map(order =>(
                                                <tr key={order.orderId}>
                                                    ...
                                                    <td><button className="btn btn-warning" onClick={()=>this.pickedUp(customer.custUsername,order.orderId,order.typeOfService,order.quantity,order.orderStatus)}>Picked Up</button></td>
